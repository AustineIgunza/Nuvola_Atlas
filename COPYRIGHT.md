...
- **2026-08-22 — working-tree note.** The August 2026 refocus deleted the prose documentation this file cites: `tasks/todo.md`, `Navuuna Build Phases.txt`, `docs/ops/*`, `docs/architecture.md`, `docs/backend/*`, `docs/data/*`, `docs/legal/*` and `docs/archive/*`. The deletion is editorial, not a disclaimer of authorship — every one of those works remains in git history and is recoverable with `git show 3a0103a -- <path>`. Category E and §11 are unchanged in substance; whoever prepares the Exhibit E deposit should export them from that commit rather than from the working tree.
