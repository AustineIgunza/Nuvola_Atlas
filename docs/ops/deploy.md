...
**Status legend:** ✅ done · ⏳ partly done (notes below the row) · 🚫 not started yet.

| # | Status | Service | Why | What you do | What you share | Where it lands | What I do next |
|---|---|---|---|---|---|---|---|
...
> **Tier 1 status notes (as of 2026-06-08):**
> - **Row 7:** the GitHub repo itself (`AustineIgunza/Nuvola_Atlas`) is ✅ — Actions CI, Dependabot, CODEOWNERS, and the per-job path filters all green. What's still missing is the branch-protection rule on `main`.
> - **Row 8:** the Vercel project is ✅ — frontend previews + production deploys land there cleanly. What's missing is the two prod env vars (`VITE_USE_REMOTE_API=true`, `VITE_API_BASE=…`); they're 🚫 because they have nowhere to point until row 4 ships a backend URL.
> - **Rows 1–6:** none of these accounts exist yet. The whole backend is still running locally on docker; nothing is deployed.
...
> **Tier 2 status notes (as of 2026-06-08):**
> - **Row 12:** the Mapbox dev token is ✅ — it's in `nuvola-atlas-frontend/.env` and renders the map locally + on Vercel previews. Outstanding piece is a separate **prod** token with a URL allowlist scoped to `atlas.<your-domain>` and `*.vercel.app`.
> - **Rows 9–11, 13:** no accounts exist yet. Postmark is the most urgent of the four because email-based 2FA cannot complete in prod without it.
...
| Status | Tool | What it unlocks | How you enable it |
|---|---|---|---|
...
| ✅ | **`gh` CLI (GitHub)** | PRs, issues, CI runs, repo settings — already authenticated for `AustineIgunza` | Already done. I use it whenever the user mentions a PR or asks about CI status. |
...
> **MCP/CLI status notes (as of 2026-06-08):**
> - `gh` CLI is the only one currently active. Everything else is 🚫 — I'm operating without Vercel/Forge/Cloudflare/Supabase API access at the moment.
> - The cheapest first unlock is **Vercel CLI + MCP** — it's a single `npm i -g vercel` + one browser OAuth and immediately covers env-var inspection, log tailing, and deploys.
